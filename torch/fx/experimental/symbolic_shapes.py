...
class ShapeEnv:
    ...
    def _rename_unbacked_to(self, orig_s: sympy.Symbol, new_s: sympy.Symbol) -> None:
        ...
        if dest is not None and free_unbacked_symbols(dest):
            # orig_s already resolves to another unbacked symbol (dest), which
            # happens when a data-dependent op's unbacked binding is rebound
            # across retrace passes (e.g. aten._unique2 re-registered on an
            # already-exported program). orig_s, new_s and dest denote the same
            # value, so unify them transitively below instead of asserting; a
            # genuine conflict still raises ValueRangeError from _set_replacement
            # (disjoint ranges). Signpost so this rare unify stays observable.
            signpost_event(
                "dynamic",
    ...
    def _unify_unbacked_aliases(self, new_s: sympy.Symbol, dest: sympy.Expr) -> None:
        """Point the unbacked aliases of a renamed symbol at one terminal.

        ``new_s``, ``dest``, and ``new_s``'s resolved replacement all denote the
        same runtime value. Redirect every unbacked alias among them onto a
        single terminal, preferring a backed one so a backed resolution is never
        downgraded to an unbacked alias and nothing is left dangling.
        """
